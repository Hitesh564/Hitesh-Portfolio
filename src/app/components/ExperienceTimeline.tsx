...
const CLR = {
  dav_school: "#c87eff",
  freelance_marketing: "#ff9e64",
  plaksha_btech: "#88c0ff",
  ml_internship: "#4dd98a",
  voice_automation: "#ff6b8b",
  ai_products: "#00f0ff",
} as const;
...
const ENTRIES: Entry[] = [
  {
    ...
    company: "DRV DAV Centenary Public School",
    period: "Apr 2022 – May 2024",
    start: new Date(2022, 3),
    end: new Date(2024, 4),
    bullets: [
      "Scored 93.2% in Class XII & 96.2% in Class X Board Examinations",
      "Core focus: Physics, Chemistry & Higher Mathematics (PCM)",
    ],
  },
  {
    ...
    bullets: [
      "Created promotional content for multiple local cafés",
      "Collaborated directly with business owners to strengthen online presence & customer engagement",
    ],
  },
  {
    id: "voice_automation",
  ...
  },
  {
    ...
    bullets: [
      "Current CGPA: 8.46 / 10.0 in CSAI program (2024 – 2028)",
      "Specializing in Generative AI, Multi-Agent Systems, RAG & Computer Vision",
    ],
  },
  {
    id: "ml_internship",
  ...
  },
  {
    ...
    bullets: [
      "AgentEval: Pip-installable SDK for LLM agent failure diagnosis (100% attribution accuracy)",
      "Veriq: Voice-enabled AI interview platform with LangGraph reasoning & RAG claim verification",
...
const ORIGIN = new Date(2022, 0, 1);
const AXIS_END = new Date(2027, 11, 1);
const TODAY = new Date();
const YEAR_MARKS = [2022, 2023, 2024, 2025, 2026, 2027];

function toMonths(d: Date | "present" | string): number {
  const dt = d === "present" || !d || !(d instanceof Date) ? TODAY : d;
  return (
    (dt.getFullYear() - ORIGIN.getFullYear()) * 12 +
    (dt.getMonth() - ORIGIN.getMonth()) +
    (dt.getDate() - 1) / 30
  );
}
...
function buildMobileDims(vpW: number, vpH: number) {
  const chartH = Math.max(vpH * CHART_MULT_V, 1300);
          ...
          >
            ...
            >
              ...
              {(() => {
                ...
                for (const e of ENTRIES) {
                  let baseTop: number;
                  if (e.id === "plaksha_btech" && typeof e.end !== "string") {
